...
class MumeStatScraper {
    ...
    parseAndSaveResults() {
        ...
        for (let i = 0; i < lines.length; i++) {
            const line = lines[i];
            const objMatch = line.match(/^Object\s+'([^']*)',\s+vnum\s+(\d+)\./i);
            const mobMatch = line.match(/^Mobile\s+'([^']*)',\s+vnum\s+(\d+)\./i);

            if (objMatch) {
                finalizeBlock();
                currentType = 'object';
                currentName = objMatch[1];
                currentVnum = parseInt(objMatch[2], 10);
                currentBlock.push(line);
            } else if (mobMatch) {
                finalizeBlock();
                currentType = 'mobile';
                currentName = mobMatch[1];
                currentVnum = parseInt(mobMatch[2], 10);
                currentBlock.push(line);
            } else {
                if (currentVnum !== null) {
                    currentBlock.push(line);
    ...
    parseEntityText(type, vnum, name, text) {
        ...
        if (type === 'object') {
            ...
            for (const line of lines) {
                ...
                const extraMatch = line.match(/Extra flags:\s*([^,\n]+)/i);
                if (extraMatch && extraMatch[1].trim().toLowerCase() !== 'none') {
                    extraFlags = extraMatch[1].split(/\s+/).map(f => f.trim()).filter(Boolean);
                }

                const wearMatch = line.match(/Wear flags:\s*([^.\n]+)/i);
                if (wearMatch && wearMatch[1].trim().toLowerCase() !== 'none') {
                    wearFlags = wearMatch[1].split(/\s+/).map(f => f.trim()).filter(Boolean);
                }
            ...
            for (const line of lines) {
                ...
                const alignMatch = line.match(/Align:\s*(-?\d+)/i);
