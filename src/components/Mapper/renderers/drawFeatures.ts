            ...
            for (let i = 0; i < bucket.length; i++) {
                    ...
                    for (const dir in ghostExits) {
                        ...
                        if (targetData && (Math.abs(targetData[2] - currentZ) <= 0.5 || ((dir === 'u' || dir === 'd') && Math.abs(targetData[2] - currentZ) <= 1.5)) && (explored.has(targetVnum) || unveilMap)) {
                            ...
                            if (hasRoadFlag && camera.zoom >= 0.1) {
                                const roadWidth = Math.min(16, 12 / camera.zoom);
                                const pathWidth = Math.min(8, 4 / camera.zoom);
                                ...
                                if (dx > 1.1 || dy > 1.1 || dir === 'u' || dir === 'd') {
                                    const connWidth = Math.min(4, 2 / camera.zoom);
                ...
                if (camera.zoom > 0.15) {
                    for (const d of ['n', 's', 'e', 'w']) {
                        ...
                        if (!hasExit) {
                            ...
                            ctx.lineWidth = Math.min(8, 2.5 / camera.zoom);
                            ctx.moveTo(x1, y1); ctx.lineTo(x2, y2); ctx.stroke();
                        } else if (hasDoor) {
                            const ddx = x2 - x1, ddy = y2 - y1; 
                            ctx.strokeStyle = isDarkMode ? "#fab387" : "#e67e22"; 
                            ctx.lineWidth = Math.min(10, 3.5 / camera.zoom);
                            ...
                            if (isClosed) { 
                                ...
                                ctx.lineWidth = Math.min(12, 4.0 / camera.zoom); 
                ...
                if (camera.zoom > 0.2) {
                    ...
                    if (mobF.length > 0 || loadF.length > 0 || questF.length > 0) {
                        ...
                        const spacing = 10 / camera.zoom;
                        for (const f of mobF) {
                            if (f.includes('AGGRESSIVE')) { ctx.fillStyle = '#f38ba8'; ctx.fillText('!', anchorX + off, anchorY); off += spacing; }
                            else if (f.includes('SHOP')) { ctx.fillStyle = '#f9e2af'; ctx.fillText('$', anchorX + off, anchorY); off += spacing; }
                            else if (f.includes('GUILD')) { ctx.fillStyle = '#cba6f7'; ctx.fillText('G', anchorX + off, anchorY); off += spacing; }
                            else if (f.includes('RENT')) { ctx.fillStyle = '#89b4fa'; ctx.fillText('R', anchorX + off, anchorY); off += spacing; }
                        }
...
export const drawLocalFeatures = (rCtx: RenderContext, localRooms: any[]) => {
    ...
    if (camera.zoom <= 0.15) {
        ctx.beginPath(); ctx.strokeStyle = "#795548"; ctx.lineWidth = Math.min(8, 2.5 / camera.zoom);
        for (const room of localRooms) {
            const vnum = String(room.id).startsWith('m_') ? room.id.substring(2) : room.id;
    ...
    if (camera.zoom > 0.2) {
        for (const room of localRooms) {
            ...
            if (mobF.length > 0 || loadF.length > 0 || questF.length > 0) {
                ...
                const spacing = 10 / camera.zoom;
                for (const f of mobF) {
                    if (f.includes('AGGRESSIVE')) { ctx.fillStyle = '#f38ba8'; ctx.fillText('!', cX + off, cY); off += spacing; }
                    else if (f.includes('SHOP')) { ctx.fillStyle = '#f9e2af'; ctx.fillText('$', cX + off, cY); off += spacing; }
                    else if (f.includes('GUILD')) { ctx.fillStyle = '#cba6f7'; ctx.fillText('G', cX + off, cY); off += spacing; }
                    else if (f.includes('RENT')) { ctx.fillStyle = '#89b4fa'; ctx.fillText('R', cX + off, cY); off += spacing; }
                }
                for (const f of questF) {
                    ctx.fillStyle = '#fab387'; 
                    ctx.fillText('?', cX + off, cY); off += spacing;
                }
