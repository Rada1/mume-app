...
- **`useStageManager.ts`**: Manages the lifecycle of capture stages (inventory, equipment, stats, etc.).
...
- **`useLineProcessor.ts`**: Converts raw lines into structured `DrawerLine` objects for UI lists.
- **`useStageInitializer.ts`**: Contains the logic for starting/stopping capture stages based on text patterns.
...
4.  The orchestrator performs final message routing and stage finalization.
