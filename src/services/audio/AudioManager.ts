...
class AudioManager {
    ...
    public async setAmbient(type: 'terrain' | 'weather' | 'zone', key: string | null, dynamicUrls?: string[], inCombat: boolean = false, isDay: boolean = true) {
        ...
        if (type === 'terrain') {
            let config = AUDIO_MANIFEST.ambient.terrains[key];
            if (!config) {
                const foundKey = Object.keys(AUDIO_MANIFEST.ambient.terrains).find(k => key.includes(k));
                if (foundKey) config = AUDIO_MANIFEST.ambient.terrains[foundKey];
            }
            ...
            const config = AUDIO_MANIFEST.ambient.weather[key];
            if (config) {
                urlToPlay = config.url;
                targetVolume = config.volume;
            }
            ...
            if (dynamicUrls && dynamicUrls.length > 0) {
                 ...
                 const config = AUDIO_MANIFEST.ambient.zones[key];
