...
ws.on('message', (data) => {
    ...
    if (state === 'LOGIN_NAME') {
        ...
        if (lower.includes('ellessar') || lower.includes('>') || lower.includes('[150:') || lower.includes('obvious exits')) {
            console.log('Successfully entered game directly. Starting commands gathering...');
            state = 'GATHERING';
            nextCommand();
        ...
        if (lower.includes('ellessar') || lower.includes('>') || lower.includes('welcome to') || lower.includes('[150:')) {
            console.log('Successfully entered game. Starting commands gathering...');
            state = 'GATHERING';
            nextCommand();
        }
        ...
        commandTimeout = setTimeout(() => {
            ...
            nextCommand();
        }, 3000); // 3 seconds timeout to handle large lists safely
    }
});
...
function finish() {
    console.log('Finished gathering all command pages. Saving to markdown...');
    let mdContent = '# MUME /lib Library Commands Reference\n\n';
    mdContent += 'This guide contains the lists of installed Mudlle behavioral libraries for rooms, mobiles, and objects, retrieved directly from the live game server.\n\n';
    ...
    for (const cmd of helpCommands) {
        ...
        const lines = text.split('\n')
            ...
            .filter(line => !line.includes('ellessar>') && !line.includes('15019[') && !line.includes('/lib commands'));
    ...
    const outputFilePath = path.join('c:/Users/pwetz/Downloads/mume app/docs', 'lib_commands_reference.md');
    fs.writeFileSync(outputFilePath, mdContent, 'utf8');
    console.log(`Saved library commands reference successfully to: ${outputFilePath}`);
    ws.close();
    process.exit(0);
}
